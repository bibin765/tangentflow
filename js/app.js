...
document.querySelectorAll('.add-block-bar button').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    switch (type) {
      ...
      case 'multi-column': block = { type: 'multi-column', text: 'Enter your text here. It will flow across multiple columns, wrapping at each column boundary. This is useful for newsletter-style layouts, academic papers, and any content that benefits from a multi-column format.', columns: 3, gap: 16 }; break
      case 'stat-row': block = { type: 'stat-row', items: 'Label: Value, Label: Value' }; break
      case 'footnote': block = { type: 'footnote', marker: '1', text: 'Footnote text goes here.' }; break
